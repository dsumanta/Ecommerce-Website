...
export const SummaryPage =()=>{
        ...
        var options = {
            ...
            handler: async function (response) {
              ...
              if(jsonRes.success){
                ...
                navigate("/order-summary",{state:{orderId:jsonRes.orderId,paymentId:jsonRes.paymentId}})
              }
            },
            prefill: {
  ...
  <div className="max-w-2xl w-full bg-white rounded-xl shadow-lg p-6">
    <h2 className="text-2xl font-bold text-gray-900 mb-6">Order Summary ({context?.productCountCart})</h2>
    ...
    {cartData && cartData.product && cartData.product.length > 0 ? ( // Check if cartData and cartData.product exist and are not empty
          cartData.product?.map((item, index) => (
            <div key={index} className="space-y-4"> {/* Add a unique key prop */}
              <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-lg">
                <img src={item?.productImage[0]} alt="Product" className="w-20 h-20 object-cover rounded-md" />
                <div className="flex-1">
                  <h3 className="font-semibold text-gray-900">{item?.productName}</h3> {/* Replace with dynamic data */}
                  <p className="text-sm text-gray-500">{`Brand: ${item?.brandName} , Quantity: ${item?.quantity}`}</p> {/* Replace with dynamic data */}
                </div>
                <p className="font-semibold text-gray-900 w-20 text-right">{DisplayINDCurrency(item?.sellingPrice * item?.quantity)}</p> {/* Replace with dynamic data */}
              </div>
            </div>
          ))
        ) : (
      ...
      <div className="mt-6 pt-6 border-t">
        <div className="flex justify-between text-base text-gray-900 mb-2">
          <p>Subtotal</p>
          <p className="font-semibold">{DisplayINDCurrency(cartData?.totalPrice)}</p>
        </div>
        ...
        <button  disabled={loader} onClick={doRazorpayTransaction} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-3 rounded-lg transition-colors">
          {loader?"Redirecting": paymentFailed?"Transaction Failed Retry": "Proced to Pay"}
        </button>
      </div>
    </div>
  </div>
